...
## レイアウト是正（egui 版 redraw 準拠への作り直し）
- 当初の実装は独自レイアウト（不透明角丸の URL バー＋36px の大タブボタン4個＋タイトル＝約140px、
  104px の分厚い2段下部バー、各コントロールが塗りつぶしボタン）で動画を大きく覆い「邪魔すぎる」状態だった。
- 「決めたレイアウト」＝ egui 版 redraw（撤去前 `src/main.rs`、`fc292b7^`）に倣って作り直した:
  - 上部 UI 帯（TOP_H=86）: 薄い半透明帯。URL 行＋ナビ行（**枠なしフラットなテキストボタン**。
    📋おすすめは候補がある時のみ・📃再生リスト/📺登録チャンネル/🕘履歴はログイン時のみ）＋タイトル行。
    タイトルが無ければ 2 行ぶんに自動短縮。
  - 下部コントローラ（BOTTOM_H=52）: 薄い半透明帯。**細い本家風シークライン**（フル幅・赤・seekable 時のみ
    可動／ライブはグレー固定）＋1 行のフラット操作（▶ 時間 👍 💬 ｜ 右寄せ 音量 🔊 コーデック 画質）。
  - ボタンは塗り背景をやめフラットなテキスト化。DirectWrite で実テキスト幅を計測してヒット矩形を算出。
  - フォントは主 22px＋小 15px の 2 系統。region_top/region_bottom も帯の実寸に縮小（動画の素通し領域が増加）。
